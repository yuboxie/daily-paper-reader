...
## 摘要
开放世界环境中的机器人操作需要跨语义、几何和长程动作动力学进行推理。现有的分层视觉-语言-动作（VLA）框架通常使用 2D 表示来连接高层推理与底层控制，但缺乏深度感知和时间一致性，限制了在复杂 3D 场景中的鲁棒性。我们提出了 ST-VLA，这是一种使用统一 3D-4D 表示来桥接感知与动作的分层 VLA 框架。ST-VLA 将 2D 指导转换为 3D 轨迹，并生成捕捉 4D 时空上下文的平滑空间掩码，为语义推理和连续控制之间提供了稳定的接口。为了实现对此类表示的有效学习，我们引入了 ST-Human，这是一个包含 14 个任务和 30 万个回合的大规模人类操作数据集，通过半自动化流水线标注了 2D、3D 和 4D 监督信息。利用 ST-Human，我们训练了 ST-VLM，这是一种时空视觉语言模型，可生成空间定位且时间相干的 3D 表示以指导策略执行。平滑的空间掩码专注于与任务相关的几何结构并稳定潜表示，从而实现在线重规划和长程推理。在 RLBench 和真实世界操作任务上的实验表明，ST-VLA 显著优于最先进的基准模型，将零样本成功率分别提高了 44.6% 和 30.3%。这些结果表明，将时空推理卸载到具有统一 3D-4D 表示的 VLM 中，可以显著提高开放世界机器人操作的鲁棒性和泛化能力。项目网站：https://oucx117.github.io/ST-VLA/。
