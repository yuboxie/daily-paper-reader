...
tags: ["query:lh-llm"]
score: 6.0
evidence: LLM推理深度和状态纠正的自适应改进
tldr: 针对大语言模型在推理时分配计算资源不均的问题，本文提出CoFiCot框架。该框架通过多指标分类器动态评估问题难度，对简单问题采用高效聚合，对复杂问题则通过基于状态的顺序修正过程进行深度推理。结合过程奖励模型（PRM），CoFiCot实现了细粒度错误定位与全局逻辑一致性的平衡，有效提升了推理效率与准确性。
selection_source: fresh_fetch
motivation: 解决推理时计算资源分配不均导致的简单任务过度计算与复杂任务修正不足的矛盾。
method: 提出一种自适应框架，利用多指标分类器分流查询，并对复杂任务实施基于历史状态的顺序修正与PRM验证。
result: 实现了根据问题难度动态调整推理策略，在保证复杂任务逻辑连贯性的同时提升了整体推理效率。
conclusion: CoFiCot通过粗细结合的自适应策略，有效弥补了无状态修正方法的局限性，优化了测试时计算资源的利用。
---
