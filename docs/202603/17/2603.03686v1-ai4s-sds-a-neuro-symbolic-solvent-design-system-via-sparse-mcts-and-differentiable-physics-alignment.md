...
title_zh: AI4S-SDS：一种基于稀疏 MCTS 和可微物理对齐的神经符号溶剂设计系统
authors: Jiangyu Chen
date: 2026-03-04
pdf: "https://arxiv.org/pdf/2603.03686v1"
tags: ["query:lh-llm"]
score: 8.0
evidence: 解决长程推理中的上下文窗口限制
...
method: 提出了一种集成稀疏MCTS、全局-局部搜索策略和可微分物理引擎的闭环神经符号框架。
result: 该系统在HSP物理约束下实现了完全的有效性，并显著提升了搜索的多样性，成功发现了一种高性能的新型光刻胶显影液。
conclusion: 结合多样性驱动的神经符号搜索与物理对齐，是加速科学发现和复杂材料设计的有效途径。
---

## 摘要
化学配方的自动化设计是材料科学的基石，但它需要在一个涉及离散成分选择和连续几何约束的高维组合空间中进行搜索。现有的语言模型（LLM）智能体在此场景下面临重大挑战，包括长程推理过程中的上下文窗口限制，以及可能导致模式崩溃的路径依赖性探索。为了解决这些问题，我们推出了 AI4S-SDS，这是一个集成了多智能体协作与定制化蒙特卡洛树搜索（MCTS）引擎的闭环神经符号框架。我们提出了一种具有动态路径重构的稀疏状态存储机制，该机制将推理历史与上下文长度解耦，从而在固定的 token 预算下实现任意深度的探索。为了减少局部收敛并提高覆盖率，我们实施了一种全局-局部搜索策略：一个记忆驱动的规划模块根据历史反馈自适应地重新配置搜索根节点，而兄弟节点感知扩展机制则在节点层面促进正交探索。此外，我们通过可微物理引擎桥接了符号推理与物理可行性，采用带有稀疏诱导正则化的混合归一化损失，在热力学约束下优化连续混合比例。实验结果表明，AI4S-SDS 在采用的基于 HSP 的物理约束下实现了完全的有效性，并且与基准智能体相比，显著提高了探索多样性。在初步的光刻实验中，该框架识别出一种新型光刻胶显影液配方，其性能优于或等同于商业基准，突显了多样性驱动的神经符号搜索在科学发现方面的潜力。
