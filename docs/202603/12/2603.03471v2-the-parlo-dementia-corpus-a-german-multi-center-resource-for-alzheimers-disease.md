...
title_zh: PARLO 痴呆语料库：一个用于阿尔茨海默病研究的德国多中心资源
authors: "Franziska Braun, Christopher Witzl, Florian Hönig, Elmar Nöth, Tobias Bocklet, Korbinian Riedhammer"
date: 2026-03-03
pdf: "https://arxiv.org/pdf/2603.03471v2"
tags: ["query:llm-mh"]
score: 7.0
evidence: 用于检测认知障碍和痴呆的语音和语言分析
tldr: 本研究针对阿尔茨海默病（AD）早期检测依赖高成本侵入性手段且缺乏非英语公开语音数据集的现状，推出了PARLO痴呆语料库（PDC）。该语料库是首个公开的德语多中心临床验证资源，涵盖了AD、轻度认知障碍患者及健康对照组在八项标准化任务中的语音录音、转录及详细临床元数据。通过基准测试，研究展示了基于语音和大语言模型进行自动化认知评估的可行性，为神经退行性疾病的多模态研究提供了重要基准。
selection_source: fresh_fetch
motivation: 旨在解决阿尔茨海默病早期检测成本高、侵入性强，且德语等非英语领域缺乏公开临床语音数据集的问题。
method: 在德国9家记忆门诊收集了AD、MCI患者及对照组在命名、流利度、图片描述和回忆等8项任务中的语音及临床元数据。
result: 构建了包含音频、人工校验转录及生物标志物信息的PDC数据集，并验证了利用LLM进行自动化认知评估的有效性。
conclusion: PDC作为首个公开的德语AD语音基准，为跨语言和多模态的神经退行性疾病研究提供了关键资源。
---

## 摘要
阿尔茨海默病（AD）的早期且易于获取的检测仍是一项重大挑战，因为目前的诊断方法通常依赖于昂贵且具有侵入性的生物标志物。语音和语言分析已成为一种极具前景的非侵入性且可扩展的认知障碍检测方法，但该领域的研究受到缺乏公开数据集的阻碍，尤其是非英语语种。本文介绍了 PARLO 痴呆语料库（PDC），这是一个全新的、经过临床验证的德国多中心 AD 资源，收集自德国的九家学术记忆诊所。该数据集包含与 AD 相关的轻度认知障碍患者、轻度至中度痴呆患者以及认知健康的对照组的语音录音。语音是通过一套包含八项神经心理学任务的标准测试组诱导产生的，包括对物命名、言语流畅性、单词重复、图片描述、故事阅读和回忆任务。除了音频录音外，该数据集还包括经过人工验证的转录文本，以及详细的人口统计学、临床和生物标志物元数据。在自动语音识别（ASR）基准测试、自动化测试评估和基于大语言模型（LLM）的分类方面的基准实验，证明了基于语音的自动认知评估的可行性，并突显了回忆驱动的语音生成的诊断价值。因此，PDC 建立了第一个公开可用的德国基准，用于神经退行性疾病的多模态和跨语言研究。
