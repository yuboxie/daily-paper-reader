...
## 摘要
基于大语言模型的深度研究智能体在处理长程信息寻求任务方面日益流行，但由于大量的推理和频繁的工具使用，它们往往会产生较高的端到端延迟。推测框架旨在通过将动作执行与推理重叠来降低延迟；然而，现有方法通常依赖于统一的推测策略和严格的动作匹配，这限制了推理加速和鲁棒性。在这项工作中，我们从动作异质性的视角重新审视了深度研究智能体的“推测-验证”范式。我们表明，Search（搜索）和 Visit（访问）动作表现出截然不同的推理和模型能力需求：基于熵的分析揭示了 Search 决策具有更高的不确定性，并能从显式推理中显著获益，而 Visit 决策的熵较低，主要取决于模型能力。受此双过程特性的启发，我们提出了 DualSpec，这是一个配备了轻量级、基于置信度的语义验证器的异构推测框架。在多个模型和基准测试上的实验表明，DualSpec 实现了高达 3.28 倍的端到端加速，同时保持了与完全推理智能体相当的准确性。
